...
use vars qw(%opt %g_opts $gameinstall $zabbixinstall $option $config $xml $xml_append $info $debug_password $iptables_restart $update_tomcat $kill $help $md5sum_files $CONFIG_FILE $XML_FILE $debug $iptables_restart1);
my $error =0;
%opt = (
	'gameinstall'      => \$gameinstall,
	'zabbixinstall'    => \$zabbixinstall,
...
sub check_opt {
	...
	if ( defined $zabbixinstall ) {
		$p{'option'} = 'zabbixinstall';
		( $option_number++ );
	}
	if ( defined $kill ) {
		$p{'option'} = 'kill';
...
sub print_opt {
	...
	print '[' . $zabbixinstall . "] \t\t->zabbixinstall\n";
	print '[' . $help . "] \t\t->help\n";
	print '[' . $kill . "] \t\t->kill\n";
