#!/bin/bash
shi_jian=`date "+%F-%T"`
Src_dir="/export/new_fabu_server/bxzw_android_server/rsync"
Src_config="/etc/fabu_server_13/ahlm2_dalu/game_config/ahlm2_andriod_xml/"
Des_dir="/home/super/update/ahlm2package"
Md5_file="/etc/fabu_server_13/ahlm2_dalu/tongbu_script/ahlm2_andriod.md5"

cp -r $Src_config /export/config_bak/ahlm2/game_config"$shi_jian"
cp -r $Src_dir /export/server_bak/ahlm2/ahlm2_android/srcfile_"$shi_jian"
curl -s http://192.168.252.240:8080/ledo_cmdb/game/xml |sed 's#\/>#\/>\n#g' > "$Src_config"t.xml

flag=$1
cd $Src_dir
find   ./ -type f -print0 | xargs -0 md5sum > $Md5_file
#for i in `cat /etc/fabu_server_13/ahlm2_dalu/tongbu_script/ahlm2_andriod_list` ; do
#{
...
sleep 10
echo -e "${1}_FAILED:`ssh -p 22 ${i} "cd ${Des_dir}/srcfile;md5sum -c ahlm2_andriod.md5  |grep -ic FAILED"`"
else
for i in `cat /etc/fabu_server_13/ahlm2_dalu/tongbu_script/ahlm2_andriod_list`; do 
{
...
sleep 10
echo -e "${i}_FAILED:`ssh -p 22 ${i} "cd ${Des_dir}/srcfile;md5sum -c ahlm2_andriod.md5  |grep -ic FAILED"`"
}&
done
fi
